...
import static com.app.banking.BankAccount.fundTransferUsingWD;
import static utils.BankingUtils.populateBankAccount;
import static utils.BankingUtils.populateHashMap;
...
//			List<BankAccount> bankAccountList = populateBankAccount();
			Map<Integer, BankAccount> hashMap = populateHashMap(populateBankAccount());
...
//						Sending int senderAccountNumber and wrapping it in ref inside bankAccountExists
						ValidationRulesBankAccount.bankAccountExists(senderAccountNumber, hashMap);
//						int index = bankAccountList.indexOf(new BankAccount(senderAccountNumber));
						BankAccount transferAccount = hashMap.get(senderAccountNumber);

						System.out.println("Enter the Reciever's account no: ");
						int recieverAccountNumber = sc.nextInt();
						ValidationRulesBankAccount.bankAccountExists(recieverAccountNumber, hashMap);
...
//						fundTransferUsingWD(bankAccountList, sender, reciever, transferAmount);
						fundTransferUsingWD(hashMap, senderAccountNumber, recieverAccountNumber,
								transferAmount);
						break;

					case 2: // list accounts
						System.out.println("List of Esteemed Accounts: ");
						for (BankAccount b : hashMap.values()) {
...
//						bankAccountList.toString();
					...
					case 3: // withdraw
						...
						ValidationRulesBankAccount.bankAccountExists(withdrawAccountNumber, hashMap);
//						int index1 = hashMap.indexOf(new BankAccount(withdrawAccountNumber));

						BankAccount withdrawAccountNumberRef = hashMap.get(withdrawAccountNumber);
...
//						BankAccount b1 = 
						...
						withdrawAccountNumberRef.withdraw(hashMap, withdrawAccountNumber, withdrawAmount);
//						int withdrawAccountNumber = sc.nextInt();
//						double withdrawAmount = sc.nextDouble();
...
//						throw new IllegalArgumentException("Unexpected value: ");
					...
					case 4: // deposit
						...
						ValidationRulesBankAccount.bankAccountExists(depositAccountNumber, hashMap);
						System.out.println("Enter funds to be deposited: ");
						double depositAmount = sc.nextDouble();
						bankAccount.deposit(hashMap, depositAccountNumber, depositAmount);
						break;
					...
					case 5: // add new account
						...
						ValidationRulesBankAccount.checkForDupAccount(hashMap, newAccountNumber);
...
//				sending data to AccountType in a all CAPS manner 
						...
						hashMap.putIfAbsent(newAccountNumber, new BankAccount(newAccountNumber, newName, newAccountType, newAccountBalance, LocalDate.now()));
//						hashMap.putif  
//						
//						(new BankAccount(newAccountNumber, newName, newAccountType,
...
//				default:
					case 6:
						...
						Map<Integer, BankAccount> sortedMap = new TreeMap<Integer, BankAccount>(hashMap);
						
						for(BankAccount b : sortedMap.values()) {
							System.out.println(b);
						}
//						Collections.sort(new TreeMap<>(hashMap));
						break;
						
					case 7:
						System.out.println("Sorted using custom sort according to AccountOpeningDate: ");
						Collections.sort(new ArrayList<BankAccount>(hashMap.values()) , new Comparator<BankAccount>() {
